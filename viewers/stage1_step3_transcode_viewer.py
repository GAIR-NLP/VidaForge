from __future__ import annotations

import pandas as pd
import streamlit as st

try:
    from .viewer_common import (
        add_fixed_range_chart,
        add_value_counts_chart,
        apply_text_probe_filters,
        ensure_size_columns,
        format_elapsed_seconds,
        render_bar_chart,
        render_dataframe_preview,
        render_json_preview,
        render_metadata_selection,
        show_samples,
        summarize_total_size,
    )
except ImportError:
    from viewer_common import (
        add_fixed_range_chart,
        add_value_counts_chart,
        apply_text_probe_filters,
        ensure_size_columns,
        format_elapsed_seconds,
        render_bar_chart,
        render_dataframe_preview,
        render_json_preview,
        render_metadata_selection,
        show_samples,
        summarize_total_size,
    )

STAGE_DIR = "stage1_ingestion"
STEP_DIR = "step3_transcode"


def render_page() -> None:
    context = render_metadata_selection(
        stage_dir=STAGE_DIR,
        step_dir=STEP_DIR,
        title="Stage 1 Step 3: Transcode",
        include_filter_scope=True,
        include_probe_filter=False,
        include_raw_dir=True,
    )
    if context is None:
        return

    frame = apply_text_probe_filters(
        context["frame"],
        keyword=context["keyword"],
        probe_filter=context["probe_filter"],
        extra_text_cols=(
            "screen_reject_reason",
            "screen_json",
            "transcode_error",
            "raw_path",
            "transcode_mode",
        ),
    )
    frame = ensure_size_columns(
        frame,
        size_bytes_col="filesize_bytes",
        size_mb_col="filesize_mb",
        path_col="video_path",
    )

    filter_scope = context["filter_scope"]
    screen_reject_reason = "all"
    transcode_scope = "all"

    with context["extra_sidebar_container"]:
        transcode_scope = st.selectbox(
            "Transcode Result",
            ("all", "ok", "failed"),
            index=0,
            key=f"{STAGE_DIR}_{STEP_DIR}_transcode_scope",
        )

        available_screen_reject_reasons: list[str] = []
        if "screen_reject_reason" in frame.columns:
            available_screen_reject_reasons = sorted(
                {
                    value
                    for value in frame["screen_reject_reason"].dropna().astype(str).tolist()
                    if value
                }
            )
        selected_screen_reject_reason = st.selectbox(
            "Reject Reason",
            ("all", *available_screen_reject_reasons),
            index=0,
            key=f"{STAGE_DIR}_{STEP_DIR}_screen_reject_reason",
            disabled=filter_scope != "reject",
            help="Set Filter Result to reject first, then filter by a specific reject reason.",
        )
        if filter_scope == "reject":
            screen_reject_reason = selected_screen_reject_reason

    if filter_scope != "all" and "screen_pass" in frame.columns:
        keep_series = pd.to_numeric(frame["screen_pass"], errors="coerce")
        if filter_scope == "keep":
            frame = frame[keep_series == 1]
        else:
            frame = frame[keep_series != 1]
        frame = frame.reset_index(drop=True)

    if screen_reject_reason != "all" and "screen_reject_reason" in frame.columns:
        frame = frame[
            frame["screen_reject_reason"].fillna("").astype(str)
            == screen_reject_reason
        ]
        frame = frame.reset_index(drop=True)

    if transcode_scope != "all" and "transcode_ok" in frame.columns:
        transcode_ok = pd.to_numeric(frame["transcode_ok"], errors="coerce")
        if transcode_scope == "ok":
            frame = frame[transcode_ok == 1]
        else:
            frame = frame[transcode_ok != 1]
        frame = frame.reset_index(drop=True)

    summary = context["summary"]

    tabs = st.tabs(["Overview", "Sample Browser"])
    with tabs[0]:
        current_total = len(frame)
        current_transcode_ok = "-"
        current_transcode_failed = "-"
        current_success_rate = "-"
        if "transcode_ok" in frame.columns:
            transcode_ok_series = pd.to_numeric(frame["transcode_ok"], errors="coerce")
            ok_count = int((transcode_ok_series == 1).sum())
            failed_count = int(current_total - ok_count)
            current_transcode_ok = ok_count
            current_transcode_failed = failed_count
            if current_total > 0:
                current_success_rate = f"{(ok_count / current_total):.2%}"

        metric_cols = st.columns(7)
        metric_cols[0].metric("Current Rows", current_total)
        metric_cols[1].metric("Transcode OK", current_transcode_ok)
        metric_cols[2].metric("Transcode Failed", current_transcode_failed)
        metric_cols[3].metric("Current Success Rate", current_success_rate)
        metric_cols[4].metric("Selected Rows", summary.get("input_count", "-"))
        metric_cols[5].metric(
            "Current Output Size",
            summarize_total_size(frame, size_bytes_col="filesize_bytes"),
        )
        metric_cols[6].metric("Elapsed", format_elapsed_seconds(summary.get("elapsed_sec")))

        preview_columns = [
            col
            for col in (
                "raw_path",
                "video_path",
                "transcode_ok",
                "transcode_error",
                "transcode_mode",
                "transcode_elapsed_sec",
                "filesize_mb",
                "duration_sec",
                "width",
                "height",
                "fps",
                "has_audio",
                "screen_pass",
                "screen_reject_reason",
            )
            if col in frame.columns
        ]
        render_dataframe_preview(frame[preview_columns], width="stretch", height=260)

        if summary:
            with st.expander("summary.json"):
                render_json_preview(summary)

        if "transcode_ok" in frame.columns:
            transcode_counts = (
                pd.to_numeric(frame["transcode_ok"], errors="coerce")
                .fillna(0)
                .astype(int)
                .map({1: "ok", 0: "failed"})
                .value_counts()
                .rename_axis("status")
                .reset_index(name="count")
            )
            render_bar_chart(
                transcode_counts,
                category_col="status",
                value_col="count",
                title="Transcode ResultDistribution（Current View）",
            )

        add_value_counts_chart(frame, "transcode_mode", "Transcode Mode Distribution")
        add_fixed_range_chart(
            frame,
            "transcode_elapsed_sec",
            "Transcode Time Distribution (sec)",
            bins=[0, 5, 15, 30, 60, float("inf")],
            labels=["0~5s", "5~15s", "15~30s", "30~60s", ">60s"],
        )
        add_fixed_range_chart(
            frame,
            "filesize_mb",
            "Output File Size Distribution (MB)",
            bins=[0, 10, 100, 1024, 10 * 1024, float("inf")],
            labels=["0~10 MB", "10~100 MB", "100 MB~1 GB", "1~10 GB", ">10 GB"],
        )
        add_value_counts_chart(frame, "screen_reject_reason", "Filter Reason Distribution", top_k=20)

        if "transcode_error" in frame.columns:
            error_frame = frame[frame["transcode_error"].fillna("").astype(str) != ""]
            if not error_frame.empty:
                add_value_counts_chart(
                    error_frame,
                    "transcode_error",
                    "Transcode Error Distribution (Top 20)",
                    top_k=20,
                )

    with tabs[1]:
        show_samples(
            frame,
            per_page=context["per_page"],
            columns=context["columns"],
            browse_order=context["browse_order"],
            widget_key_prefix=f"{STAGE_DIR}_{STEP_DIR}_samples",
            media_path_fields=("video_path", "raw_path"),
            raw_media_path_fields=("raw_path",),
            show_local_paths=context["show_local_paths"],
            detail_fields=(
                "raw_path",
                "video_path",
                "filesize_mb",
                "transcode_ok",
                "transcode_error",
                "transcode_mode",
                "transcode_elapsed_sec",
                "duration_sec",
                "width",
                "height",
                "fps",
                "has_audio",
                "screen_pass",
                "screen_reject_reason",
                "screen_json",
                "probe_ok",
                "probe_error",
            ),
        )
