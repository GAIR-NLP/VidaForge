...
def _run_ray_actor_rows(
    *,
    ...
    if stats.input_count == 0:
        ...
        writer_summary["actor_count_requested"] = actor_count
    ...
    def collect_one(
        ...
        if isinstance(actor_result, dict):
            ...
            raise TypeError(
                "Ray actor process_batch must return a dict row or a list of dict rows; "
                f"got {type(actor_result)!r}"
            )
        for output_row in output_rows:
            if not isinstance(output_row, dict):
                raise TypeError("Ray actor process_batch list result must contain dict rows")
    ...
    try:
        ...
        resolved_actor_count = resolve_replicas(
            actor_count,
            cluster_resources=ray.cluster_resources(),
            node_resources=alive_node_resources(ray.nodes()),
            cpus_per_replica=float(actor_options.get("num_cpus", 1.0)),
            gpus_per_replica=float(actor_options.get("num_gpus", 0.0)),
            max_replicas=stats.input_count,
        )
        RemoteActor = ray.remote(
            **actor_options,
            runtime_env=ray_runtime_env(),
        )(actor_cls)
        common_actor_kwargs = dict(actor_kwargs or {})
        actors = [
            RemoteActor.remote(**common_actor_kwargs)
            for _ in range(resolved_actor_count)
        ]
        row_iter = iter(rows)
    ...
    writer_summary = writer.summary()
    writer_summary["actor_count"] = resolved_actor_count
    writer_summary["actor_count_requested"] = actor_count
    writer_summary["batch_size"] = batch_size
    return writer_summary


def run_ray_actor_processing(
    *,
    input_path: Path,
    output_path: Path,
    parquet_size: int,
    ...
    writer_summary = _run_ray_actor_rows(
        rows=rows,
        stats=stats,
        output_path=output_path,
        parquet_size=parquet_size,
